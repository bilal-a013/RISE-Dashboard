...
export function SegmentedSelector<T extends string>({
  ...
  return (
    <div className="grid gap-1 rounded-2xl bg-[var(--rise-surface-soft)] p-1 sm:grid-cols-2">
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          ...
          className={cn(
            ...
            option.value === value ? "bg-[var(--rise-surface)] text-[var(--rise-purple)] shadow-sm ring-1 ring-[var(--rise-border)]" : "text-[var(--rise-text-muted)] hover:bg-[var(--rise-surface)] hover:text-[var(--rise-text)]"
          )}
        >
          {option.label}
        </button>
